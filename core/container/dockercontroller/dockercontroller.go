...
import (
	...
	container "github.com/tjfoc/tjfoc/core/container/api"
...
func (vm *DockerVM) createContainer(ctxt context.Context, client dockerClient, imageName string, containerName string, args []string,
	env []string, attachStdout bool) error {
	config := docker.Config{Cmd: args, Image: imageName, Env: env, AttachStdout: attachStdout, AttachStderr: attachStdout}
	copts := docker.CreateContainerOptions{Name: containerName, Config: &config, HostConfig: getDockerHostConfig()}
	dockerLogger.Debugf("Create container: %s", containerName)
	_, err := client.CreateContainer(copts)
	if err != nil {
		return err
	}
	dockerLogger.Debugf("Created container: %s", imageName)
	return nil
}

func (vm *DockerVM) deployImage(client dockerClient, imageName string, args []string, env []string, reader io.Reader) error {
	id := imageName
...
func (vm *DockerVM) Deploy(ctxt context.Context, containerName string, args []string, env []string, reader io.Reader) error {
	imageName, err := getImageNameFromContainerName(containerName)
	...
	switch err {
	case nil:
		if err = vm.deployImage(client, imageName, args, env, reader); err != nil {
			return err
		}
	default:
...
func (vm *DockerVM) Start(ctxt context.Context, containerName string, args []string, env []string, builder container.BuildSpecFactory, prelaunchFunc container.PrelaunchFunc) error {
	imageName, err := getImageNameFromContainerName(containerName)
	...
	if err != nil {
		...
		if err == docker.ErrNoSuchImage {
			if builder != nil {
				...
				if err1 = vm.deployImage(client, imageName, args, env, reader); err1 != nil {
					return err1
				}
...
func (vm *DockerVM) Stop(ctxt context.Context, containerName string, timeout uint, dontkill bool, dontremove bool, deleteImage bool) error {
	client, err := vm.getClientFnc()
	if err != nil {
	...
	err = vm.stopInternal(ctxt, client, containerName, timeout, dontkill, dontremove)
	if deleteImage == true {
		err = vm.Destroy(ctxt, containerName, false, false)
	}

	return err
}

func (vm *DockerVM) stopInternal(ctxt context.Context, client dockerClient, id string, timeout uint, dontkill bool, dontremove bool) error {
...
func (vm *DockerVM) Destroy(ctxt context.Context, containerName string, force bool, noprune bool) error {
	imageName, err := getImageNameFromContainerName(containerName)
	if imageName == "" && err != nil {
		return err
	}
	...
	err = client.RemoveImageExtended(imageName, docker.RemoveImageOptions{Force: force, NoPrune: noprune})

	if err != nil {
		dockerLogger.Errorf("error while destroying image: %s", err)
	} else {
		dockerLogger.Debugf("Destroyed image %s", imageName)
	}
...
// BaseImageExist 判断tjfoc-ccenv镜像是否存在
func BaseImageExist() bool {
